...
def convert_excel_to_rpy(input_path: str, output_path: str, manage_defines: bool = True):
    """将 Excel 转为 .rpy 脚本（支持多 Sheet）。

    manage_defines 为 True 且检测到 game/ 时，定义追加到 defines.rpy；
    否则定义直接写入输出脚本，便于独立使用和无副作用测试。
    """
    wb = load_workbook(input_path, data_only=True)
    ...
    all_sheet_lines = []
    all_header_lines = []
    csv_defines = []  # 收集定义，最后统一写入 defines.rpy
    ...
    for sheet in wb.worksheets:
        ...
        for row_idx, raw_row in enumerate(rows):
            ...
            if cmd == "define_character":
                ...
                csv_defines.append({"type": "character", "name": name, "line": line, "source": "excel"})
                continue
            ...
            if cmd == "define_variable":
                ...
                csv_defines.append({"type": "variable", "name": name, "line": line, "source": "excel"})
                continue
            ...
            if cmd == "define_image":
                ...
                csv_defines.append({"type": "image", "name": img_name, "line": line, "source": "excel"})
                continue
    ...
    # ── 定义落盘策略 ──
    # 在 Ren'Py 工程中先尝试追加到 game/defines.rpy；独立使用或同步失败时，
    # 定义保留在输出脚本中，确保转换结果始终可以单独使用。
    defines_rpy = _find_defines_rpy() if manage_defines else None
    embed_defines = defines_rpy is None
    if defines_rpy is not None and csv_defines:
        try:
            _append_new_defines(csv_defines)
        except Exception as e:
            embed_defines = True
            print(f"[WARN] 定义同步失败，将定义保留在输出脚本中：{e}")

    # ── 组装输出 ──
    output_parts = ["# Generated by excel_to_rpy.py",
                    "# 源文件: " + os.path.basename(input_path),
                    ""]
    if embed_defines and csv_defines:
        output_parts.extend(d["line"] for d in csv_defines)
        output_parts.append("")
    output_parts.append("")
    ...
    if defines_rpy is not None and not csv_defines:
        # 向后兼容：Excel 无定义行时，用扫描方式追加
        try:
...
def _append_new_defines(new_defines):
    """只追加尚不存在的定义，并完整保留 defines.rpy 的原始内容。"""
    ...
    if not to_add:
        print(f"[INFO] 定义管理：{len(new_defines)} 个定义全部已存在，跳过")
        return 0
    ...
    detail = f"，跳过 {skipped}" if skipped else ""
    print(f"[INFO] 定义管理：新增 {len(to_add)} 个定义{detail}")
    return len(to_add)
...
def append_defines_from_excel(excel_path):
    """从 Excel 提取定义，合并到 defines.rpy（不覆盖已有定义）。"""
    defines_rpy = _find_defines_rpy()
    if not defines_rpy:
        return
    ...
    existing = parse_existing_defines(str(defines_rpy))
    # 只保留不存在的定义（追加模式不覆盖已有）
    truly_new = [d for d in new_defines if d["name"] not in existing]
    if not truly_new:
        print(f"[INFO] 定义管理：{len(new_defines)} 个定义全部已存在于 defines.rpy")
        return

    _append_new_defines(truly_new)
...
def _interactive_rpy_to_excel():
    ...
    if not path:
        print("[ERROR] 未输入路径。")
        return
    if not os.path.isfile(path) or not path.lower().endswith(".rpy"):
...
def _interactive_roundtrip():
    """从主菜单运行自带 roundtrip 回归测试。"""
    from test_roundtrip import test_file
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_roundtrip.rpy")
    test_file(path)
