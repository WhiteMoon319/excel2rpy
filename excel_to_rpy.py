...
def convert_excel_to_rpy(input_path: str, output_path: str):
    ...
    if csv_defines:
        try:
            _merge_and_write_defines(csv_defines)
        except Exception as e:
            print(f"[INFO] 定义同步跳过: {e}")
    else:
...
def _merge_and_write_defines(new_defines):
    """合并新定义到 defines.rpy：读取现有→合并→去重→分组排序→覆写。
    同名定义以 new_defines 为准（Excel 优先）。
    """
    ...
    for name, line in existing.items():
        if "Character" in line:
            ...
            def_type = "image"
        all_defs.append({"type": def_type, "name": name, "line": line, "source": "existing"})
        seen.add(name)
    ...
    for d in new_defines:
        if d["name"] in seen:
            all_defs = [x for x in all_defs if x["name"] != d["name"]]
            updated += 1
        else:
            added += 1
        all_defs.append(d)
    ...
    grouped = _group_defines_by_character(all_defs)
    ...
    skipped = len(new_defines) - added - updated
    if skipped:
        parts.append(f"跳过 {skipped}")
    print(f"[INFO] 定义管理：defines.rpy 已更新（{', '.join(parts)}，共 {len(all_defs)} 个定义）")
...
def append_defines_from_excel(excel_path):
    ...
    _merge_and_write_defines(truly_new)
...
def _interactive_define_append():
    ...
    _merge_and_write_defines(to_add)
